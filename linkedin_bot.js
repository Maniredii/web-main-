...
async function launchBrowser(executablePath) {
    ...
    const browser = await puppeteer.launch({
        ...
        defaultViewport: {
            width: fingerprint.screen.width || Math.floor(Math.random() * (1920 - 1366 + 1) + 1366),
            height: fingerprint.screen.height || Math.floor(Math.random() * (1080 - 768 + 1) + 768),
            deviceScaleFactor: fingerprint.screen.deviceScaleFactor || 1,
            hasTouch: fingerprint.screen.hasTouch || false,
            isLandscape: fingerprint.screen.isLandscape || false,
            isMobile: false
        }
    });
